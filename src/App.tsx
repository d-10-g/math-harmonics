    ...
    <div className="w-full h-screen bg-[#050505] text-[#e0e0e0] font-sans flex flex-col overflow-hidden p-4 lg:p-6 gap-5">
      ...
      <main className={`flex-1 grid grid-cols-1 gap-5 overflow-y-auto xl:overflow-hidden custom-scrollbar ${
        ...
        <Controls
          ...
          modelBackgroundControls={<div className="space-y-2 text-xs text-white/70"><label>3D Background<select aria-label="3D Background" className="block w-full rounded bg-slate-800 p-2" value={modelBackground} onChange={e=>setModelBackground(e.target.value as BackgroundChoice)}>{(['cosmos','midnight','charcoal','slate','ivory','white','mirror','hdri'] as const).map(c=><option key={c} value={c}>{c==='hdri'?'HDRI panorama + lighting':c==='mirror'?'Mirror dome (live reflections)':c}</option>)}</select></label>{modelBackground==='hdri'&&<label className="block">Load equirectangular HDR / EXR<input aria-label="Load HDRI" type="file" accept=".hdr,.exr" onChange={e=>{const f=e.target.files?.[0];if(f){setBackgroundError('');setHdri({url:URL.createObjectURL(f),name:f.name});}e.target.value='';}}/>{hdri?.name}</label>}{backgroundError&&<p role="alert">{backgroundError}</p>}<p className="text-white/40">The mirror dome reflects the stage live. HDRIs provide panoramic backgrounds and material lighting. Gaussian splat scenes are planned for a later stage.</p></div>}
          noteSource={noteSource}
          setNoteSource={setNoteSource}
          meshUseMtl={meshUseMtl}
          setMeshUseMtl={setMeshUseMtl}
